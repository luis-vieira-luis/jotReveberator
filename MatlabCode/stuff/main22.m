...
%Cross couple input channels
for i = 1:length(input)
   crossCouple_L = (inputDelayed(i)*U(1,1) + inputDelayed(i)*U(1,2));
   CrossCouple_L(1,i) = [crossCouple_L];
   
   crossCouple_R = (inputDelayed(i)*U(2,2) + inputDelayed(i)*U(2,1));
   CrossCouple_R(1,i) = [crossCouple_R];
  
end

% Absorbent all-pass series (7 filters)
[H_L,tapoutL] = lateReverb(CrossCouple_L, lateDelaylines, ...
    gainAbsorbent,kn,attenuation);
[H_R,tapoutR] = lateReverb(CrossCouple_R, lateDelaylinesR, ...
    gainAbsorbent,kn,attenuation);
...
crossCouple_L = (H_L(1)*U(1,1) + H_R(1)*U(1,2)); 
crossCouple_R = (H_L(1)*U(2,2) + H_R(1)*U(2,1)); 
...
for n = 2:length(input)
    ...
    %left
    hN_L = inputDelayed(n) + crossCouple_L;
    hnew_L = [hN_L  inputDelayed(1,1:length(input)-1)];
    
    [H_L,tapoutL] = lateReverb(hnew_L, lateDelaylines, gainAbsorbent,kn,attenuation);
    
    %----%
    %right
    hN_R = inputDelayed(n) + crossCouple_R;
    hnew_R = [hN_R inputDelayed(1,1:length(input)-1)];
...
close all
output = input+ earlyRef*0.1 + tapout_sum(1,:);
plot(output)
%------------------------------------------------------------------------%
%% Plotting
close all
