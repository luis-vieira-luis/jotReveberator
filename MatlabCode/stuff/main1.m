...
attenuation = 10.^(attenuationDb/20); % attenuation (att)
...
kn = decayHF*[1 1 1 1 1 1 1]; 

% Absorbent all-pass series (7 filters)
[H_L,tapoutL] = lateReverb(inputDelayed, lateDelaylines, gainAbsorbent,kn,attenuation);
[H_R,tapoutR] = lateReverb(inputDelayed, lateDelaylinesR, gainAbsorbent,kn,attenuation);
...
for n = 2:length(input)
    ...
    %left
    hN_L = input(n) + crossCouple_L;
    hnew_L = [hN_L  input(1,1:length(input)-1)];
    
    [H_L,tapoutL] = lateReverb(hnew_L, lateDelaylines, gainAbsorbent,kn,attenuation);
    
    %----%
    %right
    hN_R = input(n) + crossCouple_R;
    hnew_R = [hN_R input(1,1:length(input)-1)];
